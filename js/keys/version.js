...
//
//   v5.3  theme is now a THREE-state cycle: Dark → Light → Auto (follow the OS
//         via prefers-color-scheme, tracked live). First visit still opens DARK;
//         the choice is remembered in the browser (localStorage) for later
//         visits. (v5.1 defaulted to the OS, v5.2 dropped Auto entirely — this
//         keeps dark-first AND the option to follow system settings.)
...
//   v4.8  Hide-panel button moved to the bottom of the panel; 2.5D link now opens
//         the live 2.5D tool page (not the repo); trimmed spacing to stay no-scroll.
//   v4.7  live note under the label explains Raised+Both = raised top / engraved
//         bottom (and raised+bottom becomes engraved), shown as you pick it.
//   v4.6  raised prints only on the TOP face — the bottom is always engraved (so
//         Both = raised top + engraved bottom), with a warning on raised-bottom;
//         Hide-panel button moved onto the panel with a floating Show tab; 2.5D
//         link now points to the GitHub repo; card step warns about ID badges /
//         key-tags sharing the photo.
//   v4.5  deboss: fix mirrored text (the up face is viewed from +y, so it is now
//         x-mirrored to read correctly); shrink the label to stay inside the real
//         bow outline + warn if it barely fits; rename Level/Upside-down to Teeth
//         up / Teeth down.
...
//         + keyhole-avoidance; security + key-privacy hovertext.
